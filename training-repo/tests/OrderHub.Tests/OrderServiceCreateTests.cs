...
public class OrderServiceCreateTests
{
    ...
    [Fact]
    public async Task CreateOrder_GoldTierCustomer_SnapshotsUndiscountedUnitPrice()
    {
        using var db = TestSetup.CreateContext();
        var service = TestSetup.CreateOrderService(db);
        var customer = TestSetup.AddCustomer(db, tier: CustomerTier.Gold);
        var product = TestSetup.AddProduct(db, unitPrice: 1470m);

        var result = await service.CreateOrderAsync(customer.Id, new[] { new NewOrderLine(product.Id, 1) });

        Assert.True(result.Success);
        var order = result.Value!;
        Assert.Equal(1470m, order.Items.Single().UnitPriceSnapshot);
        Assert.Equal(1323m, service.CalculateTotal(order));
    }
